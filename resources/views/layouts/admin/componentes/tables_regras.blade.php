        ...
        <tr>
            ...
            <td>
                @if(!empty($rota_exibicao))
                    <a href="{!! route($rota_exibicao,[$regra->cod_regra]) !!}">
                        <div class="media">
                            ...
                            <div class="media-body">
                                ...
                                @endif
                                <div class="text-muted smaller">Código da Regra: {!! $regra->cod_regra !!}</div>
            ...
            <td>
                @if((!empty($regra->usuario) && Auth::user()->email===$regra->usuario->email) || (Auth::user()->tipo==='Administrador' || Auth::user()->email==='jeancarlospenas25@gmail.com'))
                    @if(!empty($rota_edicao))
                        @include('componentes.link',['id' => $regra->cod_regra, 'rota' => $rota_edicao])
                    @endif

                    @if(!empty($rota_exclusao))
                        @include('componentes.form_delete',['id' => $regra->cod_regra, 'rota' => $rota_exclusao])
                    @endif
                    @if(!empty($rota_exibicao))
